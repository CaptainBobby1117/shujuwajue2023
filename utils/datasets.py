...
class ConformationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = self.data[idx].clone()                   # 根据 index 从数据集中复制一个数据出来
        if self.transform is not None:                  # CountNodesPerGraph
    ...
    def _pack_data_by_mol(self):
        ...
        if hasattr(self.data, 'idx'):
            for i in range(len(self.data)):
                #print('self.data[%d].idx.item(), self.data[%d]' % (i, i))
                #print(self.data[i].idx.item())
                #print(self.data[i])
                self._packed_data[self.data[i].idx.item()].append(self.data[i])
        else:
            for i in range(len(self.data)):
                if self._packed_data[self.data[i].smiles] != []: continue # TODO:这里改了一下，记得改回来
