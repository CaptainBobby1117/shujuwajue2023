...
def assemble_atom_pair_feature(node_attr, edge_index, edge_attr):
    h_row, h_col = node_attr[edge_index[0]], node_attr[edge_index[1]]   # [E, hidden]
    h_pair = torch.cat([h_row*h_col, edge_attr], dim=-1)    # (E, 2H) 前hidden维是边所连两点emb的对应项相乘，后H维是edge自身的emb
    return h_pair
...
def _extend_graph_order(num_nodes, edge_index, edge_type, order=3):
    ...
    N = num_nodes
    adj = to_dense_adj(edge_index).squeeze(0)                               # 将 edge_index 转换为邻接矩阵
...
def extend_graph_order_radius(num_nodes, pos, edge_index, edge_type, batch, order=3, cutoff=10.0, 
    ...
    if extend_order:
        edge_index, edge_type = _extend_graph_order(                # 转换出 edge_index 和 edge_type
            num_nodes=num_nodes, 
