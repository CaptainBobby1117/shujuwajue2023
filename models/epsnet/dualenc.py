...
def get_beta_schedule(beta_schedule, *, beta_start, beta_end, num_diffusion_timesteps): 
    ...
    if beta_schedule == "quad":         #等差数列形式([sqrt(beta_start), sqrt(beta_end)]这个区域中的num_diffusion_timesteps个数的等差数列)
        betas = (
            np.linspace(
                beta_start ** 0.5,
                beta_end ** 0.5,
    ...
    elif beta_schedule == "linear":     #等差数列形式([beta_start, beta_end]这个区域中的num_diffusion_timesteps个数的等差数列)
        betas = np.linspace(
            beta_start, beta_end, num_diffusion_timesteps, dtype=np.float64
        )
    elif beta_schedule == "const":      #常数序列（长度为num_diffusion_timesteps的全为beta_end的序列）
        betas = beta_end * np.ones(num_diffusion_timesteps, dtype=np.float64)
    elif beta_schedule == "jsd":        # 1/T, 1/(T-1), 1/(T-2), ..., 1
        betas = 1.0 / np.linspace(
            num_diffusion_timesteps, 1, num_diffusion_timesteps, dtype=np.float64
        )
    elif beta_schedule == "sigmoid":    #按照sigmoid函数取样，上界为beta_end，下界为beta_start
...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.model_type == 'diffusion':
            ...
            ## variances
            alphas = (1. - betas).cumprod(dim=0)                            #这里的 alphas 实际上是累乘后的结果，而不是我们在推导中的 α
            self.alphas = nn.Parameter(alphas, requires_grad=False)
            self.num_timesteps = self.betas.size(0)                         #num_timesteps 指beta的长度
    ...
    def forward(self, atom_type, pos, bond_index, bond_type, batch, time_step, 
        ...
        """
        N = atom_type.size(0)                                                           # 原子种类
        if edge_index is None or edge_type is None or edge_length is None:
            edge_index, edge_type = extend_graph_order_radius(                          # 扩充 edge_index, edge_type，edge_length
                num_nodes=N,
                pos=pos,
            ...
            )                                                                           #edge_index.size()=[2, E]    edge_type.size()=[E]
            edge_length = get_distance(pos, edge_index).unsqueeze(-1)   # (E, 1)        获取边长度
        local_edge_mask = is_local_edge(edge_type)  # (E, )                             # 返回 edge_type > 0, 大于0为True，否则FALSE   size()= [E]
        ...
        # Encoding global
        edge_attr_global = self.edge_encoder_global(                    # (E, hidden_size)
        ...
        # Global
        node_attr_global = self.encoder_global(                         # (N, hidden_size)
            z=atom_type,
        ...
        edge_attr_local = self.edge_encoder_local(
            edge_length=edge_length,
            edge_type=edge_type
        )   # Embed edges                                                 [E, hidden]
        # edge_attr += temb_edge

        # Local
        node_attr_local = self.encoder_local(                           # [N, hidden]
            z=atom_type,
    ...
    def get_loss_diffusion(self, atom_type, pos, bond_index, bond_type, batch, num_nodes_per_graph, num_graphs, 
                 anneal_power=2.0, return_unreduced_loss=False, return_unreduced_edge_loss=False, extend_order=True, extend_radius=True, is_sidechain=None):
        N = atom_type.size(0)           # 点的个数
        node2graph = batch              # node2graph 各个点属于哪个图

        # Four elements for DDPM: original_data(pos), gaussian_noise(pos_noise), beta(sigma), time_step
        # Sample noise levels
        time_step = torch.randint(      # timestep 是 size 为 [numgraphs//2+1] 的向量，其中为 [0, num_timesteps（beta的长度）) 的随机整数
            0, self.num_timesteps, size=(num_graphs//2+1, ), device=pos.device)
        time_step = torch.cat(          # 记 time_step 的后半为 step_2，前半为 step_1，那么 step_2[x] = num_timesteps - step_1[x]，最后截取前 num_graphs 个
            [time_step, self.num_timesteps-time_step-1], dim=0)[:num_graphs]
        a = self.alphas.index_select(0, time_step)  # (G, ) 按照上面随机的 time_step 选择对应的 alpha 并记录在 a 中
        # Perterb pos
        a_pos = a.index_select(0, node2graph).unsqueeze(-1)  # (N, 1) 将上面对于各个图随机的的 alpha 传到各个点上
        pos_noise = torch.zeros(size=pos.size(), device=pos.device)   # size 为 (N, 3) 的全 0 矩阵
        pos_noise.normal_()                                 # 初始化 pos_noise，μ=0，σ²=1 

#########################################################################################################################################################################################
        pos_perturbed = pos + pos_noise * (1.0 - a_pos).sqrt() / a_pos.sqrt()   # 加入噪音，注意的是他不是按照正常的方式加入噪音，按理说应该是给源数据一个系数，但是他把系数放到了噪音上
        #这方法导致数值实际上并不是标准化的，我不确定是否会收敛到标准高斯分布上
#########################################################################################################################################################################################
        ...
        edge_inv_global, edge_inv_local, edge_index, edge_type, edge_length, local_edge_mask = self(
            ...
            extend_radius = extend_radius,
            is_sidechain = is_sidechain
        )   # (E_global, 1), (E_local, 1), (2, E_global), (E), (E, 1), (E)

        edge2graph = node2graph.index_select(0, edge_index[0])    # (E), 这里是将所有的边对应到它所在的图中
        # Compute sigmas_edge
        a_edge = a.index_select(0, edge2graph).unsqueeze(-1)  # (E, 1)，将边所对应的 alpha 也取出来

        # Compute original and perturbed distances
        d_gt = get_distance(pos, edge_index).unsqueeze(-1)   # (E, 1)，根据各个点的位置取得各个边对应的距离，gt = ground true
        d_perturbed = edge_length                            # (E, 1)，
        # Filtering for protein
        train_edge_mask = is_train_edge(edge_index, is_sidechain)   # [E] 这个好像是用于在蛋白质合成中确定哪些边是可训练边的
        d_perturbed = torch.where(train_edge_mask.unsqueeze(-1), d_perturbed, d_gt)  # train_edge_mask 为 True 的地方取 d_perturbed ，否则取 d_gt

        if self.config.edge_encoder == 'gaussian':
            # Distances must be greater than 0 
            d_sgn = torch.sign(d_perturbed)                 # distance各个项所对应的符号
            d_perturbed = torch.clamp(d_perturbed * d_sgn, min=0.01, max=float('inf'))  # 将 d_perturbed 强行全部取绝对值放到 0.01 到 inf 的区间中
#*****************************************   TODO：之后看一下
        d_target = (d_gt - d_perturbed) / (1.0 - a_edge).sqrt() * a_edge.sqrt()     # (E_global, 1), denoising direction
#*****************************************
        global_mask = torch.logical_and(            # d_perturbed 中的项小于 cutoff 且 edge_type <= 0 (里面那个 or 的后面一项没有任何意义)
                            torch.logical_or(d_perturbed <= self.config.cutoff, local_edge_mask.unsqueeze(-1)),
                            ~local_edge_mask.unsqueeze(-1)
                        )
        target_d_global = torch.where(global_mask, d_target, torch.zeros_like(d_target))                # global_mask 为 True 的地方取 d_target，否则取 0
        edge_inv_global = torch.where(global_mask, edge_inv_global, torch.zeros_like(edge_inv_global))  # global_mask 为 True 的地方取 edge_inv_global，否则取 0
        target_pos_global = eq_transform(target_d_global, pos_perturbed, edge_index, edge_length)       # [N, 3]
        node_eq_global = eq_transform(edge_inv_global, pos_perturbed, edge_index, edge_length)          # [N, 3]
        loss_global = (node_eq_global - target_pos_global)**2                                           # 计算二者之间的距离
        loss_global = 2 * torch.sum(loss_global, dim=-1, keepdim=True)                                  # [N, 1]

        target_pos_local = eq_transform(d_target[local_edge_mask], pos_perturbed, edge_index[:, local_edge_mask], edge_length[local_edge_mask])     # [N, 3]
        node_eq_local = eq_transform(edge_inv_local, pos_perturbed, edge_index[:, local_edge_mask], edge_length[local_edge_mask])                   # [N, 3]
        loss_local = (node_eq_local - target_pos_local)**2 
        loss_local = 5 * torch.sum(loss_local, dim=-1, keepdim=True)                                                                                # [N, 1]
    ...
    def langevin_dynamics_sample_diffusion(self, atom_type, pos_init, bond_index, bond_type, batch, num_graphs, extend_order, extend_radius=True, 
        ...
        sigmas = (1.0 - self.alphas).sqrt() / self.alphas.sqrt()                    # 取 sigma 但是这里的 sigma 我不确定来源是哪里
        pos_traj = []
        ...
        with torch.no_grad():
            ...
            ## to test sampling with less intermediate diffusion steps   测试使用更少的diffusion步数
            # n_steps: the num of steps
            seq = range(self.num_timesteps-n_steps, self.num_timesteps)             # 从 num_timesteps - n_steps 开始到 num_timesteps
            seq_next = [-1] + list(seq[:-1])                                        # seq - 1
            print('pos_init')
            print(pos_init.size())
            pos = pos_init * sigmas[-1]                                             # 初始位置 * sigma[T]， pos_init 是满足标准正态分布的 [N, 3] 矩阵
            print('pos')
            print(pos.size())
            print('sigmas[-1]')
            print(sigmas[-1].size())
            if is_sidechain is not None:                                            # 如果需要仅优化 sidechain 的话，就将不是 sidechain 的部分改为 groundtrue 位置
                pos[~is_sidechain] = pos_gt[~is_sidechain]
                print('pos')
                print(pos.size())
            for i, j in tqdm(zip(reversed(seq), reversed(seq_next)), desc='sample'):            # j = i-1
                t = torch.full(size=(num_graphs,), fill_value=i, dtype=torch.long, device=pos.device)       # [G]
                ...
                # Global
                if sigmas[i] < global_start_sigma:      # 在 diffusion 去噪到一定程度之前不去考虑 global 的部分
                    ...
                    node_eq_global = 0
                # Sum
                eps_pos = node_eq_local + node_eq_global * w_global # + eps_pos_reg * w_reg 将 local 的结果和 global 的结果按比例融合
