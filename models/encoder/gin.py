...
class GINEConv(MessagePassing):
    ...
    def forward(self, x: Union[Tensor, OptPairTensor], edge_index: Adj,
        ...
        """"""
        if isinstance(x, Tensor):                               # 检测 x 是否为 Tensor 类型
            x: OptPairTensor = (x, x)                           # 变成了一对相同的 Tensor
...
class GINEncoder(torch.nn.Module):
    ...
    def forward(self, z, edge_index, edge_attr):
        ...
        for conv_idx, conv in enumerate(self.convs):
            hidden = conv(conv_input, edge_index, edge_attr)                    # [N, hidden]
            if conv_idx < len(self.convs) - 1 and self.activation is not None:  # 不是最后一层就过一次激活函数
                hidden = self.activation(hidden)
            assert hidden.shape == conv_input.shape                
            if self.short_cut and hidden.shape == conv_input.shape:
                hidden += conv_input                                            # 加入上一层的结果

            hiddens.append(hidden)
            conv_input = hidden

        if self.concat_hidden:                                                  # 若为 True 则将所有隐层接在一起
            node_feature = torch.cat(hiddens, dim=-1)
        else:
            node_feature = hiddens[-1]                                          # 取最后一层结果
