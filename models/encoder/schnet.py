...
class ShiftedSoftplus(torch.nn.Module):
    def __init__(self):
        ...
        self.shift = torch.log(torch.tensor(2.0)).item()    # ln(2.0)

    def forward(self, x):
        return F.softplus(x) - self.shift                   # 所有元素减 ln(2)
...
class CFConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_length, edge_attr):
        if self.smooth:
            ...
            C = (edge_length <= self.cutoff).float()                        # 如果 edge_length 小于 cutoff 则对应项为 1, size() = [E, 1]
        W = self.nn(edge_attr) * C.view(-1, 1)                              # 在 InteractionBlock 中的 nn 是个MLP， size() = [E, hidden(128)]

        x = self.lin1(x)                                                    # [N, hidden(128)] -> [N, hidden(128)]
        x = self.propagate(edge_index, x=x, W=W)                            # [N, hidden(128)]
        x = self.lin2(x)                                                    # [N, hidden(128)]
        return x

    def message(self, x_j, W):              # x_j [E, hidden]    W [E, hidden]
        return x_j * W
...
class InteractionBlock(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_length, edge_attr):
        x = self.conv(x, edge_index, edge_length, edge_attr)            # [N, hidden(128)]
        x = self.act(x)                                                 # 所有元素-ln(2)
        x = self.lin(x)                                                 # MLP , [N, hidden(128)]
        return x
...
class SchNetEncoder(Module):
    ...
    def forward(self, z, edge_index, edge_length, edge_attr, embed_node=True):
        if embed_node:                                              #如果为True则通过一个Embedding层
